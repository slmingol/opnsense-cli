...
async function listCerts({ filter, expiringDays } = {}) {
  ...
  if (expiringDays !== null && expiringDays !== undefined) {
    certs = certs.filter(ct => {
      const d = daysLeft(ct);
      return d !== null && d !== undefined && d <= expiringDays;
    });
  }

  if (certs.length === 0) {
    console.log(filter || (expiringDays !== null && expiringDays !== undefined) ? 'No certificates match the filter.' : 'No certificates found.');
    return;
  }
...
module.exports = { listCerts, importCert, deleteCert, checkCerts, fmtExpiry, tsToMs, fmtDate, daysLeft };
