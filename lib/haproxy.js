...
async function resolveHostnameForIp(ip) {
  ...
  } catch { /* hostname lookup failed — no match */ }
  return null;
}

async function getServerUuidsForBackend(client, name) {
  const bResp   = await client.post('/api/haproxy/settings/searchBackends', SEARCH_BODY);
  const backend = (bResp.data.rows || []).find(b => b.name === name);
  if (!backend) {
    console.error(`No backend named "${name}" found.`);
    process.exit(1);
  }
  return new Set((backend.linkedServers || '').split(',').filter(Boolean));
}

async function fixBackendDnsAddresses({ apply = false, name = null } = {}) {
  const client = getClient();

  let servers = (await client.post('/api/haproxy/settings/searchServers', SEARCH_BODY)).data.rows || [];
  if (name) {
    const uuids = await getServerUuidsForBackend(client, name);
    servers = servers.filter(s => uuids.has(s.uuid));
  }
...
// ---------------------------------------------------------------------------
// Convert hostname server addresses to static IPs
// ---------------------------------------------------------------------------
...
async function fixBackendIpAddresses({ apply = false, name = null } = {}) {
  ...
  for (const server of servers) {
    if (IP_RE.test(server.address)) continue;
    let ip = null;
    try { ip = (await dns.lookup(server.address, 4)).address; } catch { /* unresolvable — skip */ }
    if (ip) toUpdate.push({ server, ip });
    else skipped.push(server);
  }
  ...
  if (toUpdate.length === 0 && skipped.length === 0) {
    console.log(`${c.green}Nothing to do -- all server addresses are already IPs.${c.reset}\n`);
    return;
  }

  console.log(`\n${c.bold}Will convert (${toUpdate.length}):${c.reset}`);
  for (const { server, ip } of toUpdate) {
    console.log(`  ${c.cyan}${server.name}${c.reset}  ${c.gray}${server.address}:${server.port}${c.reset}  →  ${c.green}${ip}:${server.port}${c.reset}`);
  }
  ...
  for (const { server, ip } of toUpdate) {
    try {
      ...
      console.log(`  ${c.green}✓${c.reset} ${server.name}  ${c.gray}${server.address}${c.reset} → ${ip}`);
      ok++;
    } catch (err) {
      console.log(`  ${c.yellow}✗${c.reset} ${server.name}  ${err.response?.data?.message || err.message}`);
      fail++;
...
// ---------------------------------------------------------------------------
// Clear resolver setting on servers
// ---------------------------------------------------------------------------
...
async function disableBackendResolver({ apply = false, name = null } = {}) {
  ...
  const toUpdate = servers.filter(s => s.resolver && s.resolver !== '');

  console.log(`\n${c.bold}HAProxy resolver disable${apply ? '' : ' (dry-run)'}${c.reset}`);
  console.log(c.gray + '─'.repeat(72) + c.reset);

  if (toUpdate.length === 0) {
    console.log(`${c.green}Nothing to do -- no servers with resolver configured.${c.reset}\n`);
    return;
  }

  console.log(`\n${c.bold}Will clear resolver (${toUpdate.length}):${c.reset}`);
  for (const s of toUpdate) {
    console.log(`  ${c.cyan}${s.name}${c.reset}  resolver=${c.yellow}${s.resolver}${c.reset}  →  ${c.green}(none)${c.reset}`);
  }
  ...
  for (const server of toUpdate) {
    try {
      ...
      console.log(`  ${c.green}✓${c.reset} ${server.name}  resolver cleared`);
      ok++;
    } catch (err) {
      console.log(`  ${c.yellow}✗${c.reset} ${server.name}  ${err.response?.data?.message || err.message}`);
      fail++;
...
async function inspectBackend({ name }) {
  ...
  const sResp      = await client.post('/api/haproxy/settings/searchServers', SEARCH_BODY);
  const linkedUuids = (backend.linkedServers || '').split(',').filter(Boolean);
  const servers    = linkedUuids.map(uuid => (sResp.data.rows || []).find(s => s.uuid === uuid)).filter(Boolean);

  console.log(JSON.stringify({ backend, servers }, null, 2));
}
...
async function applyHaproxy() {
  const client = getClient();
  await client.post('/api/haproxy/service/reconfigure');
  console.log(`  ${c.green}✓${c.reset} HAProxy applied`);
}

async function restartHaproxy() {
  const client = getClient();
  await client.post('/api/haproxy/service/restart');
  console.log(`  ${c.green}✓${c.reset} HAProxy restarted`);
}

// ---------------------------------------------------------------------------
// Audit backends: classify servers as static IP, service hostname, or shared
// ---------------------------------------------------------------------------

async function auditBackends() {
  const client = getClient();

  const [sResp, hostResp, aliasResp] = await Promise.all([
    client.post('/api/haproxy/settings/searchServers', SEARCH_BODY),
    client.post('/api/unbound/settings/searchHostOverride', SEARCH_BODY),
    client.post('/api/unbound/settings/searchHostAlias',   SEARCH_BODY),
  ]);
  const servers  = sResp.data.rows    || [];
  const hosts    = hostResp.data.rows || [];
  const aliases  = aliasResp.data.rows || [];

  // Count aliases per host UUID → FQDN lookup
  const aliasCount = {};
  for (const a of aliases) {
    aliasCount[a.host] = (aliasCount[a.host] || 0) + 1;
  }

  // Build FQDN → alias count map
  const hostAliasCount = {};
  for (const h of hosts) {
    const fqdn = `${h.hostname}.${h.domain}`;
    hostAliasCount[fqdn] = aliasCount[h.uuid] || 0;
  }
  ...
  for (const server of servers) {
    ...
    if (IP_RE.test(addr)) {
      ...
      shared.push({ server, aliasCount: hostAliasCount[addr] });
    } else {
      named.push(server);
    }
  }
  ...
  if (shared.length > 0) {
    ...
    for (const { server, aliasCount: ac } of shared) {
      console.log(`  ${c.yellow}⚠${c.reset}  ${c.cyan}${server.name}${c.reset}`);
      console.log(`     ${c.yellow}${server.address}:${server.port}${c.reset}  ${c.gray}primary host with ${ac} aliases${c.reset}`);
    }
  }
  ...
  if (named.length > 0) {
    ...
    for (const s of named) {
      console.log(`  ${c.blue}~${c.reset}  ${c.cyan}${s.name}${c.reset}`);
  ...
  if (safe.length > 0) {
    ...
    for (const s of safe) {
      console.log(`  ${c.green}✓${c.reset}  ${c.cyan}${s.name}${c.reset}  ${s.address}:${s.port}`);
    }
  }
...
async function listFrontends({ filter } = {}) {
  ...
  for (const [i, f] of frontends.entries()) {
    ...
    const cert = f.sslCertificateRef ? `${c.yellow}[SSL: ${f.sslCertificateRef}]${c.reset}` : '';

    console.log(`${c.gray}${i + 1}.${c.reset} ${c.bold}${c.cyan}${f.name}${c.reset}  ${c.gray}uuid=${f.uuid}${c.reset}`);
...
async function addFrontend({ name, bind, mode = 'http', certName = null }) {
  ...
  const fResp    = await client.post('/api/haproxy/settings/searchFrontends', SEARCH_BODY);
  const existing = (fResp.data.rows || []).find(f => f.name === name);
  if (existing) {
    console.log(`  ${c.blue}ℹ${c.reset} Frontend already exists: ${c.cyan}${name}${c.reset}`);
    return;
  }

  const payload = { frontend: { enabled: '1', name, bind, mode } };

  if (certName) {
    const certsResp = await client.get('/api/haproxy/settings/get');
    const certs     = Object.values(certsResp.data.haproxy?.sslCertificates || {});
    const cert      = certs.find(c => c.description === certName || c.refid === certName);
    if (!cert) throw new Error(`Certificate not found: ${certName}`);
    payload.frontend.sslCertificateRef = cert.refid;
    payload.frontend.ssl = '1';
  }

  await client.post('/api/haproxy/settings/addFrontend', payload);
...
async function deleteFrontend(name) {
  ...
  const fResp    = await client.post('/api/haproxy/settings/searchFrontends', SEARCH_BODY);
  const frontend = (fResp.data.rows || []).find(f => f.name === name);
  if (!frontend) throw new Error(`Frontend not found: ${name}`);

  await client.post(`/api/haproxy/settings/delFrontend/${frontend.uuid}`);
...
async function assignFrontendCert({ name, certName }) {
  ...
  const certsResp = await client.get('/api/haproxy/settings/get');
  const certs     = Object.values(certsResp.data.haproxy?.sslCertificates || {});
  const cert      = certs.find(ct => ct.description === certName || ct.refid === certName);
  if (!cert) throw new Error(`Certificate not found: ${certName}`);

  await client.post(`/api/haproxy/settings/setFrontend/${frontend.uuid}`, {
    frontend: { ...frontend, sslCertificateRef: cert.refid, ssl: '1' },
  });
  console.log(`  ${c.green}✓${c.reset} Frontend ${c.cyan}${name}${c.reset} cert → ${c.yellow}${certName}${c.reset}`);
  await applyChanges(client);
}

module.exports = { listBackends, addBackend, deleteBackend, addFrontendRoute, deleteFrontendRoute, fixBackendDnsAddresses, fixBackendIpAddresses, disableBackendResolver, inspectBackend, applyHaproxy, restartHaproxy, auditBackends, listFrontends, addFrontend, deleteFrontend, assignFrontendCert };
