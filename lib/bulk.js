...
const { addEntry, addAlias, getEntries } = require('./dns');
const { addBackend } = require('./haproxy');
...
// Backend host/domain a service record resolves to.
// Precedence: per-record fields > SERVICES_HOST / SERVICES_DOMAIN env vars >
// historical defaults (kept for backward compatibility with existing files).
function serviceTarget(s) {
  return {
    host:   s.host   || s.host_bub || process.env.SERVICES_HOST   || 'docker-host-01-svcs',
    domain: s.domain || process.env.SERVICES_DOMAIN || 'bub.lan',
  };
}
...
function previewServices(services) {
  ...
  for (const s of services) {
    const { host, domain } = serviceTarget(s);
    console.log(`  ${c.cyan}${s.alias}${c.reset}  port=${s.port}  "${s.description}"`);
    console.log(`    ${c.gray}DNS  ${s.alias}.${domain} → ${host}.${domain}${c.reset}`);
    console.log(`    ${c.gray}HAProxy  ${s.alias} → ${s.alias}.${domain}:${s.port}${c.reset}`);
  }
}
...
async function applyServices(services, backend) {
  console.log(`\n${c.bold}[Services]${c.reset}`);
  let ok = 0, fail = 0;
  for (const s of services) {
    const { host, domain } = serviceTarget(s);
    const fqdn = `${s.alias}.${domain}`;
    const ssl  = s.ssl === true || s.ssl === 'true';
    console.log(`\n  ${c.cyan}${s.alias}${c.reset}  port=${s.port}${ssl ? ` ${c.yellow}[SSL]${c.reset}` : ''}`);
    try {
      await addAlias({ host, domain, aliasHost: s.alias, aliasDomain: domain, description: s.description, backend });
      console.log(`    ${c.green}✓${c.reset} DNS ${fqdn}`);
      await addBackend({ name: s.alias, serverName: fqdn, serverAddress: fqdn, serverPort: parseInt(s.port), ssl });
...
async function applyDNS(entries, backend) {
  console.log(`\n${c.bold}[DNS]${c.reset}`);
  let ok = 0, fail = 0;
  for (const d of entries) {
    try {
      await addEntry({ host: d.host, domain: d.domain, ip: d.ip, description: d.description || '', backend });
...
async function bulkImport({ file, dryRun = false, backend }) {
  ...
  if (data.services?.length) { const r = await applyServices(data.services, backend); totalOk += r.ok; totalFail += r.fail; }
  if (data.dns?.length)      { const r = await applyDNS(data.dns, backend);           totalOk += r.ok; totalFail += r.fail; }
...
async function hasPlugin(client, name) {
  const resp = await client.get('/api/core/firmware/info');
  return (resp.data.plugin || []).some(p => p.name === name && p.installed === '1');
}

async function bulkExport({ output, backend } = {}) {
  const client = getClient();

  // DNS host overrides (from the configured DNS backend)
  const dns = await getEntries(backend);

  // HAProxy backends + servers + frontends — only when the plugin is installed
  let haproxy = [], frontends = [];
  if (!await hasPlugin(client, 'os-haproxy')) {
    // Warn on stderr so stdout stays valid JSON when no --output file is given
    console.error(`${c.yellow}⚠ os-haproxy plugin not installed — skipping HAProxy export${c.reset}`);
  } else {
    ...
    haproxy = (bResp.data.rows || []).flatMap(b =>
    ...
    frontends = (fResp.data.rows || []).map(f => ({
      name:    f.name,
...
module.exports = { bulkImport, bulkExport, parseJSON, parseCSV, validate, serviceTarget };
