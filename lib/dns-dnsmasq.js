...
// ---------------------------------------------------------------------------
// Export (normalized entries for bulk operations)
// ---------------------------------------------------------------------------
...
async function getEntries() {
  ...
  const rows   = await searchHosts(client);
  return rows.map(r => {
    const entry = { host: r.host, domain: r.domain, ip: r.ip };
    if (r.descr) entry.description = r.descr;
    return entry;
  });
}

module.exports = { splitList, hostPayload, listEntries, addEntry, updateEntry, deleteEntry, addAlias, deleteAlias, getEntries };
