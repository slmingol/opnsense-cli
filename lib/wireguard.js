...
async function _applyFirewall(client) {
...
async function listTunnels() {
  ...
  } catch { /* live handshake data unavailable — display static config only */ }
...
async function applyProtonVPN({
  ...
  console.log(`${c.blue}[3/6]${c.reset} Gateway ${gwName}`);
  {
    ...
    if (exists) {
      await client.post(`/api/routing/settings/set_gateway/${exists.uuid}`, payload);
      console.log(`  ${c.green}✓${c.reset} Updated gateway ${gwName} → ${gatewayIP}`);
    } else {
      await client.post('/api/routing/settings/add_gateway', payload);
...
module.exports = { parseWireGuardConf, deriveGatewayIP, listTunnels, applyProtonVPN, teardownProtonVPN };
